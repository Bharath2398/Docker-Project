...
const mongodb_client = new MongoClient("mongodb://mongodb:27017",{useUnifiedTopology:true});
...
app.get('/account/create',async function (req,res) {
  try {
    result = await mongodb_client.db('crm').collection('accounts').insertOne({id:req.query.id,name:req.query.name});
...
app.get('/account/update',async function (req,res) {
  try {
    result = await mongodb_client.db('crm').collection('accounts').updateOne({id:req.query.id},{$set:{name:req.query.name}});
...
app.get('/account/delete',async function (req,res) {
  try {
    result = await mongodb_client.db('crm').collection('accounts').deleteOne({id:req.query.id});
...
app.get('/dashboard',async function (req,res) {
  try {
    accounts = await mongodb_client.db('crm').collection('accounts').find().toArray();
